...
class Config_Transformer(BaseModel):
    """
    Configuration for the voice-based heart failure prediction challenge.
    ...
    n_layer: int = Field(
        default=2,
        description="Nombre de blocs de transformer empilés")
    ...
    num_heads: int = Field(
        default=7,
        description="Nombre de têtes du Transformer")

    block_size:int = Field(
        default=4,
        description="Contexte size")
...
def get_config_Transformer() -> Config_Transformer:
    """
    Get the configuration instance.
